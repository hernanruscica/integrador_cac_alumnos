...
module.exports = {
    index: (req, res) => {res.render('index', {mensaje: null})},
    registrar_form: (req, res) => {res.render('register', {mensaje : null})},    
    login: (req, res) => {req.session.usuario ? res.redirect('/contactos') : res.render('login', {mensaje: null}); },
    autenticar: (req, res) => {
            ...
            if (!err){                
                if (results.length > 0){
                    ...
                    if (resultadoComparacion == true){
                        ...
                        res.render('login', {mensaje: 'contraseniaerronea'})
                    }
                }else{ 
                    console.log("no encontró ningún usuario con ese nombre");                    
                    res.render('login', {mensaje: 'usuarioinexistente'});
                }                
            }else{
                console.log("Error en la busqueda del usuario en la BD");
                res.render('login', {mensaje: 'errorbd'});                              
            }
        })
    },
            ...
            if (!err){
                ...
                indexModel.addOne(formData, conexion, (err, results) => {
                    if (!err){
                        ...
                        res.render('login', {mensaje: "ingresocorrecto"});
                    } else {
                        console.log(`Error al registrar usuario en la bd: "${err.sqlMessage}"`);                                                
                        res.render('register', {mensaje: "yaexiste"});
                    }
                })
            } 
        });        
    ...
    recuperar_verform: async (req, res) => {
        ...
        if (idUsuario === undefined){
            res.render('recovery_form', {mensaje: null});
        }else{
                ...
                if (results.length > 0){                    
                    ...
                    await indexModel.updateIdRecovery(usuario.id, id_recuperacion, conexion,  (err, results) => {
                        if (!err) {
                            ...
                            res.render('index', {mensaje: "correoenviado"});
                        }else{
                            console.log(err);
                        }
                    });                        
                    ...
                    res.render('recovery_form', {mensaje: 'usuarioinexistente'});
                }
            }else{
